...
public sealed class Day17 : Solver
{
    ...
    protected override Task<object> Part1() => Task.FromResult(play(2022).ToString() as object);

    protected override Task<object> Part2() => Task.FromResult(play(1000000000000).ToString() as object);
    ...
    long play(long rounds) 
    {
        ...
        for (int round = 0; round < rounds; round++) {
            ...
            for (int i = 0; i < 4; i++) {
                py--;
                if (RawInput[w] == '<' && !tetris.simpleCheckX(px - 1, py, p)) px--;
                if (RawInput[w] == '>' && !tetris.simpleCheckX(px + 1, py, p)) px++;
                w = (w + 1) % size;
            }
            while (!tetris.checkCollisionsY(p, px, py)) {
                py--;
                if (RawInput[w] == '<' && !tetris.checkCollisionsX(p, px - 1, py, tetris.ltab)) px--;
                if (RawInput[w] == '>' && !tetris.checkCollisionsX(p, px + 1, py, tetris.rtab)) px++;
                w = (w + 1) % size;
            }
            tetris.drop(p, px, py);
